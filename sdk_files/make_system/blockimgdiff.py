...
def BBOTAExtraInfo(PARTITION, 
                   tgt_name, src_name, 
                   tgt_ranges, src_ranges, 
                   tgt_size, src_size, 
                   style, SIZE, OTHERS):
  return ('BBOTA|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s' % (PARTITION, 
                                                   tgt_name, src_name, 
                                                   str(tgt_ranges) if tgt_ranges else '', 
                                                   str(src_ranges) if src_ranges else '', 
                                                   str(tgt_size), str(src_size), 
                                                   style, str(SIZE), OTHERS))
...
class BlockImageDiff(object):
  ...
  def Compute(self, prefix, blockimg_pieces=1):
    ...
    # check number of new transfer          03/12/2019@chenyuqin
    if blockimg_pieces > 1:
      new_xf_number = 0
      ...
      if new_xf_number > 1:
        blockimg_pieces = 1
        print ('warning: blockimg_pieces fall back to 1, for there are %d new transfers' % (new_xf_number,))

    self.ComputePatches(prefix, blockimg_pieces)
    self.WriteTransfers(prefix, blockimg_pieces)

    # Report the imgdiff stats.
    if not self.disable_imgdiff:
      self.imgdiff_stats.Report()

  def WriteTransfers(self, prefix, blockimg_pieces):
    def WriteSplitTransfers(out, style, target_blocks, blockimg_pieces=1):
      ...
      if style == 'new' and blockimg_pieces > 1:
        blocks_limit = target_blocks.size() / blockimg_pieces           # blocks_limit supposed > 0
        total = 0
        index = 0
        while target_blocks:
          if index == (blockimg_pieces - 1):
            blocks_to_write = target_blocks.first(target_blocks.size())
          else:
          ...
          index += 1
        assert index == blockimg_pieces, 'index (%d) != blockimg_pieces (%d)' % (index, blockimg_pieces)
        return total
      else:
        blocks_limit = 1024
        total = 0
    ...
    for xf in self.transfers:
      ...
      if xf.style == "new":
        ...
        assert tgt_size == WriteSplitTransfers(out, xf.style, xf.tgt_ranges, blockimg_pieces)
        total += tgt_size
      elif xf.style == "move":
        assert xf.tgt_ranges
      ...
      if free_string:
        ...
        out.extend(free_string)     # loki: extend() instead of append() for freeing duplicated properly
    ...
    out_origin = out[:]     # added for freeing duplicated properly (only keep the last one, NOTICE that might increase the actual max_stashed_blocks)      12/22/2016@chenyuqin
    out = []
    freed_stashs = []
    index = len(out_origin)
    while index > 0:
      index = index - 1
      cmd = out_origin[index]
      if cmd.startswith('free '):
        if cmd in freed_stashs:
          continue
        freed_stashs.append(cmd)
      out.append(cmd)
    ...
    with open(prefix + ".transfer.list", "wb") as f:
      for i in out:
        f.write(i)
    if out_origin != out:
      with open(prefix + ".transfer.list.origin", "wb") as f:
        for i in out_origin:
          f.write(i)
  ...
  def ComputePatches(self, prefix, blockimg_pieces):
    ...
    with open(prefix + ".new.dat", "wb") as new_f:
      for index, xf in enumerate(self.transfers):
        if xf.style == "zero":
          ...
          if blockimg_pieces > 1:
            target_blocks = xf.tgt_ranges
            blocks_limit = target_blocks.size() / blockimg_pieces

            # save the 1st piece as 'system.new.dat' (synchronized with 'WriteSplitTransfers()')
            blocks_to_write = target_blocks.first(blocks_limit)
            self.tgt.WriteRangeDataToFd(blocks_to_write, new_f)
            target_blocks = target_blocks.subtract(blocks_to_write)
            ...
            while target_blocks:
              if index == (blockimg_pieces - 1):
                ...
                blocks_to_write = target_blocks.first(blocks_limit)
              with open(prefix + ".new.dat." + str(index), "wb") as new_f_piece:
                self.tgt.WriteRangeDataToFd(blocks_to_write, new_f_piece)
              target_blocks = target_blocks.subtract(blocks_to_write)
              index += 1
          ...
          logger.info(
              "%10d %10d (%6.2f%%) %7s %s %s", tgt_size, tgt_size, 100.0,
              xf.style, xf.tgt_name, str(xf.tgt_ranges))
          common.OPTIONS.extra_info.append(BBOTAExtraInfo(os.path.basename(prefix), 
                                                          xf.tgt_name, '', 
                                                          xf.tgt_ranges, None, 
                                                          tgt_size, 0, 
                                                          xf.style, tgt_size, 
          ...
          if xf.src_sha1 == xf.tgt_sha1:
            ...
            xf.patch_info = None
            tgt_size = xf.tgt_ranges.size() * self.tgt.blocksize
            src_size = xf.src_ranges.size() * self.src.blocksize
            if xf.src_ranges != xf.tgt_ranges:
              logger.info(
              ...
              common.OPTIONS.extra_info.append(BBOTAExtraInfo(os.path.basename(prefix), 
                                                              xf.tgt_name, xf.src_name, 
                                                              xf.tgt_ranges, xf.src_ranges, 
                                                              tgt_size, src_size, 
                                                              xf.style, 0, 
                                                              ('from %s to %s' % (str(xf.src_ranges), str(xf.tgt_ranges)))))
          else:
            if xf.patch_info:
    ...
    with open(prefix + ".patch.dat", "wb") as patch_fd:
      for index, patch_info, _ in patches:
        ...
        common.OPTIONS.extra_info.append(BBOTAExtraInfo(os.path.basename(prefix), 
                                                        ...
                                                        xf.tgt_ranges, xf.src_ranges, 
                                                        tgt_size, src_size, 
                                                        xf.style, xf.patch_len, 
                                                        ('%.2f%%' % (xf.patch_len * 100.0 / tgt_size,))))
      ...
      while True:
        ...
        if not patch_info:
          ...
          try:
            ...
            # if imgdiff failed, please try to use bsdiff processing. add by zhuangyan 2018/01/09
            if imgdiff:
              print("warning: Failed to generate imgdiff for %s: tgt=%s, src=%s"%(
                        xf.tgt_name if xf.tgt_name == xf.src_name else
                            xf.tgt_name + " (from " + xf.src_name + ")", 
                        xf.tgt_ranges, xf.src_ranges))
              xf.style = 'bsdiff'
              try:
                patch_info = compute_patch(src_file, tgt_file, False)
              except ValueError as e:
                with lock:
                  error_messages.append(
                    "Error: Failed to generate bsdiff for %s: tgt=%s, src=%s:\n%s" % (
                        xf.tgt_name if xf.tgt_name == xf.src_name else
                            xf.tgt_name + " (from " + xf.src_name + ")",
  ...
  def FindTransfers(self):
    ...
    # add by zhuangyan for oem.zip 
    if cache_size:
      max_blocks_per_transfer = int(cache_size * split_threshold /
                                  self.tgt.blocksize)
    else:
      max_blocks_per_transfer = 0
    empty = RangeSet()
