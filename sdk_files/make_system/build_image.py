...
UUID = "73fbcbc8-c941-55c6-a927-5eaa41dad5dd" # what-
HASH_SEED = "cf1dd641-c549-596c-a5b8-aa0103173ba3" # hash_seed-what-
...
def GetDiskUsage(path):
  ...
  """
  cmd = ["du", "-k", "-s", os.path.realpath(path)]
...
def GetInodeUsage(path):
  ...
  """
  cmd = ["find", os.path.realpath(path), "-print"]
  output = common.RunAndCheckOutput(cmd, verbose=False)
...
def BuildImageMkfs(in_dir, prop_dict, out_file, target_out, fs_config):
  ...
  # 'Use a fixed timestamp (01/01/2009) when packaging the image.' if timestamp not specified (except userdata.img)       11/21/2017@chenyuqin
  if 'timestamp' not in prop_dict.keys() and prop_dict['mount_point'] != 'data':
    prop_dict["timestamp"] = FIXED_FILE_TIMESTAMP
  ...
  if fs_type.startswith("ext"):
    ...
    if prop_dict["ext_mkuserimg"] == "mkuserimg_mke2fs":
      ...
      '''
      # we need to fixed UUID/HASH_SEED to reduce package size.
      # add by zhuangyan 2019/04/17
      build_command.extend(["-U", UUID])
      build_command.extend(["-S", HASH_SEED])
...
def ImagePropFromGlobalDict(glob_dict, mount_point):
  ...
  if mount_point == "system":
  ...
  elif mount_point == "logdata":
    copy_prop("logdata_fs_type", "fs_type")
    copy_prop("logdata_size", "partition_size")
    copy_prop("logdata_selinux_fc", "selinux_fc")
  elif mount_point == "factory":
    copy_prop("fs_type", "fs_type")
    copy_prop("factory_size", "partition_size")
    copy_prop("factory_selinux_fc", "selinux_fc")
    ...
    copy_prop("avb_oem_salt", "avb_salt")
    copy_prop("fs_type", "fs_type")
    ...
    copy_prop("avb_oem_partition_name", "avb_partition_name")
  elif mount_point == "vgc" or mount_point == "vgc_ex":
    copy_prop("avb_vgc_add_hashtree_footer_args",
              "avb_add_hashtree_footer_args")
    copy_prop("avb_vgc_key_path", "avb_key_path")
    copy_prop("avb_vgc_algorithm", "avb_algorithm")
    copy_prop("avb_vgc_salt", "avb_salt")
    copy_prop("fs_type", "fs_type")
    if mount_point == "vgc":
      copy_prop("avb_vgc_hashtree_enable", "avb_hashtree_enable")
      copy_prop("vgc_size", "partition_size")
    elif mount_point == "vgc_ex":
      copy_prop("vgc_ex_size", "partition_size")
      d["mount_point"] = "vgc"
      mount_point = "vgc"
    if not copy_prop("vgc_journal_size", "journal_size"):
      d["journal_size"] = "0"
    copy_prop("vgc_verity_block_device", "verity_block_device")
    copy_prop("vgc_extfs_inode_count", "extfs_inode_count")
    if not copy_prop("vgc_extfs_rsv_pct", "extfs_rsv_pct"):
      d["extfs_rsv_pct"] = "0"
    copy_prop("vgc_reserved_size", "partition_reserved_size")
    copy_prop("vgc_selinux_fc", "selinux_fc")
  elif mount_point == "dyn":
    copy_prop("avb_dyn_hashtree_enable", "avb_hashtree_enable")
    copy_prop("avb_dyn_add_hashtree_footer_args",
              "avb_add_hashtree_footer_args")
    copy_prop("avb_dyn_key_path", "avb_key_path")
    copy_prop("avb_dyn_algorithm", "avb_algorithm")
    copy_prop("avb_dyn_salt", "avb_salt")
    copy_prop("fs_type", "fs_type")
    copy_prop("dyn_size", "partition_size")
    if not copy_prop("dyn_journal_size", "journal_size"):
      d["journal_size"] = "0" 
    copy_prop("dyn_verity_block_device", "verity_block_device")
    copy_prop("dyn_extfs_inode_count", "extfs_inode_count")
    if not copy_prop("dyn_extfs_rsv_pct", "extfs_rsv_pct"):
      d["extfs_rsv_pct"] = "0" 
    copy_prop("dyn_reserved_size", "partition_reserved_size")
    copy_prop("vgc_selinux_fc", "selinux_fc")
...
def main(argv):
  ...
  if "mount_point" in glob_dict:
    ...
    if image_filename == "system.img":
      ...
      mount_point = "cache"
    elif image_filename == "logdata.img":
      mount_point = "logdata"
    elif image_filename == "vendor.img":
      mount_point = "vendor"
    elif image_filename.startswith('odm'):
      mount_point = "odm"
    ...
    elif image_filename == "vgc.img":
      mount_point = "vgc"
    elif image_filename == "vgc_ex.img":
      mount_point = "vgc_ex"
    elif image_filename == "factory.img":
      mount_point = "factory"
    elif image_filename == "dyn.img":
      mount_point = "dyn"
    else:
