...
class Options(object):
  ...
  def __init__(self):
    ...
    self.logfile = None
    self.oem_diff = False #force use oem.diff instead of oem_zip
    self.vendor = None
    self.extra_script = None
    self.extra_files = None
    self.odm_rz = False
    self.rollback_index = None

    self.temp_dir = None      # using specified temp_dir instead of '/tmp'      10/18/2017@zhuangyan
    self.allow_non_ab = None

OPTIONS = Options()

# add for OTA remote signing    11/21/2017@chenyuqin
OPTIONS.signer_server = 'compiler213'
OPTIONS.signer_base = ''
OPTIONS.signer_base_boot = ''

# add for oem project           11/21/2017@chenyuqin
OPTIONS.dict_oem_project = os.getenv('BBK_OEM_PROJECTS', None)
OPTIONS.dict_oem_dir = []

# add for vgc project           10/23/2019@wuquan
OPTIONS.dict_vgc_project = os.getenv('BBK_VGC_PROJECTS', None)
OPTIONS.dict_vgc_dir = []

# add for dyn project           10/23/2019@wuquan
OPTIONS.dict_dyn_project = os.getenv('VIVO_DYN_PROJECTS', None)
OPTIONS.dict_dyn_dir = []
OPTIONS.dict_odm_project = os.getenv('BBK_ODM_PROJECTS', None)
OPTIONS.dict_odm_dir = []

# The block size that's used across the releasetools scripts.
BLOCK_SIZE = 4096

# add for extra info used to analyse status of package        05/08/2018@chenyuqin
OPTIONS.extra_info = []

# add for updating block-based full OTA package in multi-thread         03/12/2019@chenyuqin
OPTIONS.blockimg_pieces = 16
...
PARTITIONS_WITH_CARE_MAP = ('system', 'vendor', 'product', 'system_ext', 'odm','vgc','oem')
...
def set_oem_vgc_infos(type):
  # transform OPTIONS.dict_oem_project into OPTIONS.dict_oem_dir
  # transform OPTIONS.dict_vgc_project into OPTIONS.dict_vgc_dir
  name = "dict_%s_project"%type
  dir_name = "dict_%s_dir"%type
  if OPTIONS.__dict__[name] is not None:
    for project in OPTIONS.__dict__[name].split(' '):
      if project != '':
        pieces = list(project.split(','))
        dir = type.upper() + '_' + '_'.join(pieces)
        OPTIONS.__dict__[dir_name].append(dir)
    OPTIONS.__dict__[dir_name] = list(set(OPTIONS.__dict__[dir_name]))
    logger.info("--- %s info ---"%type)
    logger.info(str(OPTIONS.__dict__[dir_name]))

def LoadInfoDict(input_file, repacking=False, getExtra=False):
  ...
  if not OPTIONS.dict_oem_project and 'BBK_OEM_PROJECTS' in d:
    OPTIONS.dict_oem_project = d['BBK_OEM_PROJECTS']
    set_oem_vgc_infos('oem')
  if not OPTIONS.dict_vgc_project and 'BBK_VGC_PROJECTS' in d:
    OPTIONS.dict_vgc_project = d['BBK_VGC_PROJECTS']
    set_oem_vgc_infos('vgc')
  if 'BBK_OTA_ARGS' in d and not OPTIONS.extra_script and getExtra:
    extra_check = False
    for each in d['BBK_OTA_ARGS'].split(' '):
      each = each.strip()
      if each:
        if each == "-n":
          continue
        elif each.startswith("--extra_file=") and type(OPTIONS.extra_files) is dict:
          each=each[len("--extra_file="):]
          src, dest = each.split(":")
          OPTIONS.extra_files[src] = dest
        elif each == "-e":
          extra_check = True
        elif extra_check:
          assert not OPTIONS.extra_script, "cannot exist more than one extra_script"
          OPTIONS.extra_script = each
          extra_check = False
  if not OPTIONS.dict_dyn_project and 'VIVO_DYN_PROJECTS' in d:
    OPTIONS.dict_dyn_project = d['VIVO_DYN_PROJECTS']
    set_oem_vgc_infos('dyn')
  ...
  # add for oem properties          11/21/2017@chenyuqin
  if OPTIONS.dict_oem_project != None:
    for oem_dir in OPTIONS.dict_oem_dir:
      print ('load oem properties for %s' % (oem_dir,))
      d['build.prop' + '_' + oem_dir] = LoadOemProp(read_helper, oem_dir)

  # add for vgc properties          08/02/2019@wuquan
  if OPTIONS.dict_vgc_project != None:
    for vgc_dir in OPTIONS.dict_vgc_dir:
      print ('load vgc properties for %s' % (vgc_dir,))
      d['build.prop' + '_' + vgc_dir] = LoadVgcProp(read_helper, vgc_dir)

  # add for dyn properties          04/014/2021@zhanglingyao
  if OPTIONS.dict_dyn_project != None:
    for dyn_dir in OPTIONS.dict_dyn_dir:
      print ('load dyn properties for %s' % (dyn_dir,))
      d['build.prop' + '_' + dyn_dir] = LoadDynProp(read_helper, dyn_dir)

  # add for odm properties          22/05/2021@zhanglingyao
  if d['odm.build.prop'].GetProp('ro.odm.rz') == 'yes':
    OPTIONS.odm_rz = True
  else:
    assert not OPTIONS.dict_odm_project, 'ro.odm.rz!=yes, BBK_ODM_PROJECTS set'

  if OPTIONS.dict_odm_project != None:
    for odm_dir in OPTIONS.dict_odm_dir:
      print ('load odm properties for %s' % (odm_dir,))
      d['build.prop' + '_' + odm_dir] = LoadOdmProp(read_helper, odm_dir)
...
def LoadOemProp(read_helper, oem_dir):
  try:
    fn = oem_dir + '/' + 'oem.prop'
    data = read_helper(fn)
  except KeyError:
    print ("Warning: could not find %s in %s" % (fn, zip))
    data = ""
  return LoadDictionaryFromLines(data.split("\n"))

def LoadVgcProp(read_helper, vgc_dir):
  try:
    fn = vgc_dir + '/' + 'vgc.prop'
    data = read_helper(fn)
  except KeyError:
...
def LoadVgcEnvProp(read_helper, vgc_dir):
  try:
    fn = vgc_dir + '/' + 'env.prop'
    data = read_helper(fn)
  except KeyError:
...
def LoadVgcVendorEnvProp(read_helper, vgc_dir):
  try:
    fn = vgc_dir + '/' + 'vendor_env.prop'
    data = read_helper(fn)
  except KeyError:
    print ("Warning: could not find %s in %s" % (fn, zip))
    data = ""
...
def LoadDynProp(read_helper, dyn_dir):
  try:
    fn = dyn_dir + '/' + 'dyn.prop'
    data = read_helper(fn)
  except KeyError:
...
def LoadOdmProp(read_helper, odm_dir):
  try:
    fn = odm_dir + '/etc/build.prop'
    data = read_helper(fn)
  except KeyError:
    print ("Warning: could not find %s in %s" % (fn, zip))
    data = ""
...
def LoadDictionaryFromLines(lines):
  ...
  for line in lines:
    ...
    if "=" in line:
      ...
      if name == 'ro.build.fingerprint' and d.has_key(name):
        continue
      d[name] = value
  return d
...
class PartitionBuildProps(object):
  ...
  def FromInputFile(input_file, name, placeholder_values=None):
    ...
    if name.upper() == 'SYSTEM':
      for prop_file in ['META/oem.prop', 'META/vgc.prop', 'META/env.prop', 'META/vendor_env.prop']:
        try:
          data += ReadFromInputFile(input_file, prop_file)
          break
        except KeyError:
  ...
  def _LoadBuildProp(self, data):
    for line in data.split('\n'):
      ...
      if line.startswith("import"):
        if '/oem/oem.prop' not in line and '/vgc/vgc.prop' not in line and '/vgc/env.prop' not in line and '/vgc/vendor_env.prop' not in line:
...
def BuildVBMeta(image_path, partitions, name, needed_partitions, props=[]):
  ...
  orig_partition = name
  type = ""
  if name.startswith("vbmeta_vgc_"):
    orig_partition = "vbmeta_vgc"
    type = "vgc"
  elif name.startswith("vbmeta_oem_"):
    orig_partition = "vbmeta_oem"
    type = "oem"
  elif name.startswith("vbmeta_odm_"):
    orig_partition = "vbmeta_odm"
    type = "odm"
  avbtool = OPTIONS.info_dict["avb_avbtool"]
  cmd = [avbtool, "make_vbmeta_image", "--output", image_path]
  AppendAVBSigningArgs(cmd, orig_partition)
  ...
  args = OPTIONS.info_dict.get("avb_{}_args".format(orig_partition))
  ...
  if type in ["vgc", "oem"]:
      for each in props:
          tmp_args = ["--prop", "ro.%s.avbtool.test:%s"%(type, each)]
          cmd.extend(tmp_args)
...
def ParseOptions(argv,
  ...
  try:
    opts, args = getopt.getopt(
        ...
        ["help", "verbose", "path=", "signapk_path=",
         ...
         "aftl_manufacturer_key_path=", "aftl_signer_helper=", "temp_dir=", "blockimg_pieces="] +
  ...
  for o, a in opts:
    if o in ("-h", "--help"):
      ...
      OPTIONS.temp_dir = a
      if not os.path.exists(OPTIONS.temp_dir): os.mkdir(OPTIONS.temp_dir)
    elif o in ("--blockimg_pieces",):
      if a.isdigit():
        OPTIONS.blockimg_pieces = int(a)
      else:
        raise ValueError("Cannot parse value %r for option %r - only "
                         "integers are allowed." % (a, o))
      if OPTIONS.blockimg_pieces > 16:
        OPTIONS.blockimg_pieces = 16            # max pieces number
        print ('warning: adjust OPTIONS.blockimg_pieces to 16')
    else:
  ...
  if not OPTIONS.temp_dir:
    temp_dir_check = os.path.join('/home', 'compiler', 'workspace/gphone/.tmp_upgrade')
    if os.path.isdir(temp_dir_check):
        print("Warning: use temp dir: %s" % temp_dir_check)
        OPTIONS.temp_dir = temp_dir_check
...
def MakeTempFile(prefix='tmp', suffix=''):
  ...
  fd, fn = tempfile.mkstemp(prefix=prefix, suffix=suffix, dir=OPTIONS.temp_dir)
  os.close(fd)
...
def MakeTempDir(prefix='tmp', suffix=''):
  ...
  """
  dir_name = tempfile.mkdtemp(suffix=suffix, prefix=prefix, dir=OPTIONS.temp_dir)
...
def get_oem_vgc_path(partition, solution):
  # add to support updating oem.img temporarily (for partition 'oem' not included in 'recovery.fstab')    11/21/2017@chenyuqin
  if partition.startswith('oem') or partition.startswith('OEM') or partition.startswith('odm'):
    device_path = '/dev/block/bootdevice/by-name/oem'
    if solution == 'MTK':
      device_path = '/dev/block/by-name/cust'
    elif solution == 'SAMSUNG':
      device_path = '/dev/block/platform/13100000.ufs/by-name/oem'
  elif partition.startswith('vgc') or partition.startswith('VGC'):
    device_path = '/dev/block/bootdevice/by-name/vgc'
    if solution == 'MTK':
      device_path = '/dev/block/by-name/vgc'
    elif solution == 'SAMSUNG':
      device_path = '/dev/block/platform/13100000.ufs/by-name/vgc'
    if partition == 'vgc_ex' or partition.endswith('_EX'):
      device_path = '%s_ex'%device_path
  return '"%s"'%device_path

def get_solution():
    solution = OPTIONS.vendor if OPTIONS.vendor else ''
    if 'build.prop' in OPTIONS.info_dict:
      solution = OPTIONS.info_dict['build.prop'].GetProp('ro.vivo.product.solution')
    return solution

class BlockDifference(object):
  def __init__(self, partition, tgt, src=None, check_first_block=False,
               version=None, disable_imgdiff=False, vgc_dynamic=False):
    self.tgt = tgt
    self.src = src
    self.partition = partition
    self.check_first_block = check_first_block
    self.disable_imgdiff = disable_imgdiff
    self.vgc_dynamic = vgc_dynamic
    self.blocking_pars = ['system', 'vendor', 'product', 'system_ext']
    if not OPTIONS.odm_rz:
      self.blocking_pars.append('odm')
    ...
    # b.Compute(self.path)
    if self.src == None and partition in self.blocking_pars:
      b.Compute(self.path, OPTIONS.blockimg_pieces)
    else:
      b.Compute(self.path)
    ...
    if is_dynamic or self.vgc_dynamic:
      if partition.startswith('oem') or partition.startswith('OEM'):
          device_path = 'oem'
          if solution == 'MTK':
              device_path = 'cust'
      elif partition.startswith('vgc') or partition.startswith('VGC'):
          device_path = 'vgc'
      elif partition.startswith('odm') or partition.startswith('ODM'):
          device_path = 'odm'
      else:
          device_path = partition
      self.device = 'map_partition("%s")' % device_path
    else:
      # add to support updating oem.img temporarily (for partition 'oem' not included in 'recovery.fstab')    11/21/2017@chenyuqin
      if partition.upper().startswith('OEM') or partition.upper().startswith('VGC'):
        device_path = get_oem_vgc_path(partition, solution)
      elif partition.upper().startswith('DYN'):
        device_path = '"/data/vivo-common/dyn/system_dyn.img"'
      else:
        if OPTIONS.source_info_dict is None:
          _, device_path = GetTypeAndDeviceExpr("/" + partition,
                                              OPTIONS.info_dict)
        else:
          _, device_path = GetTypeAndDeviceExpr("/" + partition,
  ...
  def WriteScript(self, script, output_zip, progress=None,
    ...
    if progress:
      script.ShowProgress(progress, progress * 400)
    # self._WriteUpdate(script, output_zip)
    if self.src == None and self.partition in self.blocking_pars:
      self._WriteUpdate(script, output_zip, OPTIONS.blockimg_pieces)
    else:
      self._WriteUpdate(script, output_zip)
  ...
  def WriteVerifyScript(self, script, touched_blocks_only=False, cota=False):
    if self.partition.startswith('OEM'):
      partition = ('oem/' + self.partition)
    elif self.partition.startswith('VGC'):
      partition = ('vgc/' + self.partition)
    elif self.partition.startswith('DYN'):
      partition = ('dyn/' + self.partition)
    elif self.partition.startswith('ODM_'):
      partition = ('odm/' + self.partition)
    else:
      partition = self.partition
    ...
    # incremental OTA
      ...
      device = '"/data/cota/cota_vgc.img"' if cota else self.device
      ranges_str = ranges.to_string_raw()
      script.AppendExtra(
          'if (range_sha1(%s, "%s") == "%s" || block_image_verify(%s, '
          'package_extract_file("%s.transfer.list"), "%s.new.dat", '
          '"%s.patch.dat")) then' % (
              device, ranges_str, expected_sha1,
              device, partition, partition, partition))
      ...
      if self.version >= 4:
        ...
        if self.check_first_block:
          script.AppendExtra('check_first_block(%s);' % (device,))
        ...
        script.AppendExtra((
            ...
            'endif;').format(device=device, ranges=ranges_str,
  ...
  def _WriteUpdate(self, script, output_zip, blockimg_pieces=1):
    saved_self_partition = self.partition
    if self.partition.startswith('OEM'):
      self.partition = ('oem/' + self.partition)
    elif self.partition.startswith('VGC'):
      self.partition = ('vgc/' + self.partition)
    elif self.partition.startswith('DYN'):
      self.partition = ('dyn/' + self.partition)
    elif self.partition.startswith('ODM_'):
      self.partition = ('odm/' + self.partition)
    ZipWrite(output_zip,
    ...
    if not self.src and ((self.partition.startswith('oem') or blockimg_pieces == 1) or not os.path.isfile('%s.new.dat.1' % (self.path))):
      ...
      if blockimg_pieces > 1 and self.partition in self.blocking_pars:            # partition not necessary
        index = 1
        while index < blockimg_pieces:
          assert os.path.isfile('%s.new.dat.%d' % (self.path, index))
          ZipWrite(output_zip,
                   '%s.new.dat.%d' % (self.path, index),
                   '%s.new.dat.%d' % (self.partition, index))
          index += 1
    ...
    if self.partition in self.blocking_pars:
      script.Print('tsaltes 0x2E707521 1')                                                  # assume device partition corrupted     02/21/2019@chenyuqin
    script.AppendExtra(script.WordWrap(call))
    if self.partition in self.blocking_pars:
      script.Print('tsaltes 0x2E707521 0')
...
def MakeSurvivalPatch(input_dir, output_sink, survival_img, boot_img,
                      info_dict=None):

  if info_dict is None:
    info_dict = OPTIONS.info_dict

  full_survival_image = info_dict.get("full_survival_image") == "true"

  if full_survival_image:
    output_sink("etc/ww.img", survival_img.data)
    ...
    if system_root_image:
      ...
      if os.path.exists(path):
        diff_program.append("-b")
        diff_program.append(path)
        bonus_args = "--bonus /system/etc/recovery-resource.dat"       # loki: recovery.img and survival.img share the same recovery-resource.dat
      else:
        bonus_args = ""

    d = Difference(survival_img, boot_img, diff_program=diff_program)
    _, _, patch = d.ComputePatch()
    output_sink("wwh", patch)                                   # 'survival-from-boot.p' -> 'wwh', to maintain secrecy

  try:
    # The following GetTypeAndDevice()s need to use the path in the target
    # info_dict instead of source_info_dict.
    boot_type, boot_device = GetTypeAndDevice("/boot", info_dict)
    survival_type, survival_device = 'EMMC', '/dev/block/bootdevice/by-name/survival'       # FIXED for survival as no entry matched in fstab
  except KeyError:
    return
...
""" % {'type': survival_type,
       'device': survival_device,
       'sha1': survival_img.sha1,
       'size': survival_img.size}
  else:
    sh = """#!/system/bin/sh
if ! applypatch --check %(survival_type)s:%(survival_device)s:%(survival_size)d:%(survival_sha1)s; then
  applypatch %(bonus_args)s \\
          --patch /system/wwh \\
          --source %(boot_type)s:%(boot_device)s:%(boot_size)d:%(boot_sha1)s \\
          --target %(survival_type)s:%(survival_device)s:%(survival_size)d:%(survival_sha1)s && \\
      log -t recovery "Installing new ww image: succeeded" || \\
      log -t recovery "Installing new ww image: failed"
else
  log -t recovery "ww image already installed"
...
""" % {'boot_size': boot_img.size,
       'boot_sha1': boot_img.sha1,
       'survival_size': survival_img.size,
       'survival_sha1': survival_img.sha1,
       'boot_type': boot_type,
       'boot_device': boot_device,
       'survival_type': survival_type,
       'survival_device': survival_device,
       'bonus_args': bonus_args}

  sh_location = "bin/ww"                                        # 'bin/install-survival.sh' -> 'bin/ww', to maintain secrecy
...
class DynamicPartitionsDifference(object):
  def __init__(self, info_dict, block_diffs, progress_dict=None,
               source_info_dict=None, use_vgc=False, use_oem=False, use_odm=False):
    self.use_vgc = use_vgc
    self.use_oem = use_oem
    self.use_odm = use_odm
    if not OPTIONS.odm_rz:
      assert not use_odm, "ro.odm.rz!=yes, use_odm true"
    self.source_info_dict = source_info_dict
    ...
    solution = get_solution()
    oem_par = "oem" if solution != "MTK" else "cust"
    self.separate_pars = ['vgc', oem_par]
    if OPTIONS.odm_rz:
      self.separate_pars.append('odm')
    self.oem_par = oem_par
    tmp = []
    for e in block_diffs:
        partition = e.partition
        if partition.upper().startswith("VGC"):
            partition = 'vgc'
        elif partition.upper().startswith("OEM"):
            partition = oem_par
        elif partition.upper().startswith("ODM"):
            partition = 'odm'
        tmp.append((partition, e))
    ...
    for g in tgt_groups:
      for p in shlex.split(info_dict.get(
        ...
        if p not in self.separate_pars and not use_vgc and not use_oem and not use_odm:
        ...
        elif (use_vgc and p == 'vgc') or (use_oem and p == oem_par) or (use_odm and p == 'odm'):
            self._partition_updates[p].tgt_group = g
    ...
    block_diffs_with_target = set(p for p, u in self._partition_updates.items()
                                  if u.tgt_size)
    if not use_vgc and not use_oem and not use_odm:
        if 'vgc' in target_dynamic_partitions:
            target_dynamic_partitions.remove('vgc')
        if oem_par in target_dynamic_partitions:
            target_dynamic_partitions.remove(oem_par)
        if OPTIONS.odm_rz and 'odm' in target_dynamic_partitions:
            target_dynamic_partitions.remove('odm')
        self.all_partitions = [each for each in target_dynamic_partitions]
    ...
    block_diffs_with_source = set(p for p, u in self._partition_updates.items()
                                  if u.src_size)
    if not use_vgc and not use_oem and not use_odm:
        if 'vgc' in source_dynamic_partitions:
            source_dynamic_partitions.remove('vgc')
        if oem_par in source_dynamic_partitions:
            source_dynamic_partitions.remove(oem_par)
        if OPTIONS.odm_rz and 'odm' in source_dynamic_partitions:
            source_dynamic_partitions.remove('odm')
  ...
  def WriteScript(self, script, output_zip, write_verify_script=False, dynamic_file=None):
    ...
    if not dynamic_file:
        dynamic_file = "dynamic_partitions_op_list"
    ZipWrite(output_zip, op_list_path, dynamic_file)

    script.Comment('Update dynamic partition metadata')
    script.AppendExtra('assert(update_dynamic_partitions('
                       'package_extract_file("%s")));'%dynamic_file)
  ...
  def _Compute(self):
    ...
    if self._remove_all_before_apply and not self.use_vgc and not self.use_oem and not self.use_odm:
      comment('Remove all existing dynamic partitions and groups before '
              'applying full OTA')
      #append('remove_all_groups')
      for each in self.all_partitions:
          append('remove %s'%each)
    elif self.use_vgc and not self.source_info_dict:
      append('remove vgc')
    elif self.use_oem and not self.source_info_dict:
      append('remove %s'%self.oem_par)
    elif self.use_odm and not self.source_info_dict:
      append('remove odm')
